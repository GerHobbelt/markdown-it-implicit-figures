...
(function (global, factory) {
  ...
  (global = global || self, global.markdownitImplicitFigures = factory());
  ...
  function tokenizeImagesAndMedia(state, silent, md) {
    ...
    if (!silent) {
      ...
      state.md.inline.parse(content, state.md, state.env, tokens = []); // html5-media changes start here! ///////////////////////////////////////
      ...
      if (mediaType === 'image') {
        attrs.push(['alt', '']);
      } // actual value is set in renderer.js
  ...
  function implicitFiguresPlugin(md, options) {
    ...
    md.core.ruler.before('replacements', 'implicit_figures', implicitFigures); // from html5-media: replace image rule with images and media function

    md.inline.ruler.at('image', (tokens, silent) => tokenizeImagesAndMedia(tokens, silent, md)); // from html5-media: set render rules for video and audio tokens to an anonymous function that calls renderMedia()
